...
`ctx.agents`（[发起者作用域](../s12-subagents/README.zh.md)）始终只携带发起本次操作的那个 `Agent`——从不携带轮次或取消 signal。这是设计笔记明确点出的边界：如果把轮次生命周期的状态加进驱动器生命周期的环境上下文里，会让一个陈旧的异步后代看起来仍然对*之后*的某个轮次保有权限。因此取消只有一个所有者，并且只通过显式参数传递，从不通过环境查找。

## 协作式工具取消

轮次取消停止的是循环本身；工具取消是下一层对应的机制，作用在[第 6 章](../s06-tool-pipeline/README.zh.md)所描述的工具注册表边界上。同样的「请求停止而非杀死」哲学在这里适用，并带有它自己的类型层保证。
