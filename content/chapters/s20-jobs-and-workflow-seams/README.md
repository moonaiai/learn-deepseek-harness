...
summary: ctx.jobs and ctx.workflowEngine are real capability seams with Definition/Provider/Consumer
  roles for fire-and-forget background work and worker-thread orchestration (plus
  its fixed Ralph policy); Schedule sits beside them with the same background-work
  shape but is explicitly not a seam — no ctx key, durable state directly in the session
  log.
seamKind: seam
