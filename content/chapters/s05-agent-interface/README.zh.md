...
`dsh-agent`(`packages/core/agent`)是所有插件真正面向编程的接口——`Agent` 类型、挂在 `ctx.agents` 上的 `AgentRegistry` 服务,以及 `agent/*` 事件词汇表——它对实现它的循环**没有任何依赖**。这种拆分正是设计的核心:一个 UI、ACP 桥接层或编排器调用 `ctx.agents.get(id)` 时,完全不需要知道那个 id 背后是 `AgentLoop`、一个测试替身,还是未来某种别的驱动器。本章覆盖这个包所拥有的四样东西:`Agent` 契约、注册表生命周期及其工厂模式、进程本地的发起方作用域,以及 `AgentHandle` 的所有权模型。

## 一览

四个术语撑起整章。其中两个是 handle 和它的目录;另外两个是层叠在其下的、更微妙的所有权与身份机制。

:::concept{term="Agent"}
循环无关的 handle:身份、options、session、inbox、status、一个带作用域的 `ctx`,以及 `cancel`/`whenIdle`/`send` 和 `followup`/`steer`/`inject` 这几个预设。它不提及任何轮次/步骤机器——任何满足这份接口的对象(包括手写的测试替身)都是合法的 `Agent`。
:::

:::concept{term="AgentRegistry(ctx.agents)"}
进程中所有存活 `Agent` 的实时目录,同时也是工厂与发起方作用域机制的载体。它对外的查询面很小、且是同步的;而*构造*则被委托给任何实现了 `AgentFactory` 的插件。
:::

:::concept{term="AgentHandle"}
一种 dispose 能力,而不是查询结果。任何人通过 `ctx.agents.get(id)` 查到的仍是裸 `Agent`;只有从 `create()`/`resume()` 拿到 `AgentHandle` 的那个调用方才握有 `dispose()`。
:::

:::concept{term="发起方作用域"}
一个 `AsyncLocalStorage<Agent>`,回答"当前这条异步调用链,*作为一个值*,其主体是哪一个 `Agent`",而不需要让驱动器之下的每一个辅助函数都显式转发一个 `agent` 参数。它严格限定在进程内部。
:::
...
:::decision
接口与驱动器之所以是两个独立的包,是因为它们被独立地拥有和演进:`core/agent` 持有每个消费方都面向编程的契约,而 `core/agent-loop` 持有该契约的一份可替换实现。一个想要发消息、取消运行、观察状态变化的消费方,只面向这份稳定的契约编程,从不 import 驱动器——因此驱动器可以被替换,而任何一处调用点都不需要改动。
:::
...
agent 自己的、带作用域的 Cordis context。任何通过 `agent.ctx` 注册的东西——工具、提示词片段、监听器——都只对这一个 agent 生效,并在该 agent dispose 时自动全部撤销。这正是上一章讨论循环时提到的 `dsh-scope` 机制;`dsh-agent` 是它的两个消费方之一(另一个是 `system-prompt`)。
:::
...
:::timeline
- setup(agentCtx)——在任何东西对外可见之前组合作用域世界(工具、提示词片段、监听器);受信任、只负责组合
- enter()——权威的 id 冲突检查;在*不发出通知*的情况下插入条目
- announce()——稍后恰好发出一次 `agent/created`;一个同步的 detach 会被推迟到这次分发完全展开之后
- agent/session-start——一个同步、不可 veto 的通知(它不能为启动设置门禁)
- 第一次提示词组装——agent 此时完全上线
:::
...
:::concept{term="AgentHandle"}
一种 dispose 能力,而不是查询结果。任何人通过 `ctx.agents.get(id)` 查到的仍然是裸 `Agent`;只有从 `create()`/`resume()` 拿到 `AgentHandle` 的那个调用方才握有 `dispose()` 这个 disposer。
:::
...
:::fold[teardown:在"关闭"与"排空"之间刻意保持不对称,以及重入逃生口]
