...
skill 能力用大型 API 常用的两级技巧，解决"不该塞进每次提示词的指令"这个问题：廉价的 `name`/`description` 目录先进入会话，只有当模型判断某个技能确实适用时，才通过 `skill(name)` 工具调用取回完整指令正文。`ctx.skills` 是 Service Definition——一个合并任意 provider 注册的注册表；`dsh-skill-filesystem` 是内置 provider，负责从磁盘发现 `SKILL.md` bundle;`dsh-tool-skill` 是 Consumer，发布会话目录并拥有 `skill` 工具。下文讲清注册表契约、分层合并规则、文件系统发现 provider，以及端到端的目录/加载流程。

## 速览

:::concept{term="ctx.skills / SkillRegistry"}
Service Definition:provider 注册表——合并任意注册的 provider、裁决重名，并暴露摘要与完整定义。它从不知道某个技能来自本地目录、HTTP 端点，还是插件内嵌的数据。
:::

:::concept{term="两级拆分"}
本章的核心模式，借用大型 API 发布索引的手法：廉价的 `name`/`description` 目录先发布；昂贵的完整指令正文只在需要时通过 `skill(name)` 调用加载。
:::
...
provider 契约:`list()` 返回 `SkillCandidate[]` 或一份部分观测；`get(candidate)` 返回 `SkillDefinition`。耗时或远程的工作属于被 await 的 `list()` 内部，绝不在同步工厂函数里。
:::
