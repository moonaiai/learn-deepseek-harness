...
`todo_write` and plan mode are the two mechanisms in this module that are *not* capability seams: each has exactly one implementation, no Service Provider to swap in, and no deploy-time choice about which backend handles it. Both are assembled entirely from three primitives earlier chapters already built — a `SessionEventMap` member, whole-value-replace folding over it, and (plan mode only) a `ctx.systemPrompt` section. What they add on top is durable, human-watchable collaboration state: a task list a UI can render beside the conversation, and a mode flag that changes what the model is told before it acts. Read on for how each is put together from those primitives, and why neither needed a fourth role.

## At a glance

:::concept{term="todo_write"}
One tool, `todo_write(todos: [{ content, status }])`, registered on `ctx.tools`. The defining rule is whole-list replacement: every call sends the complete list and replaces whatever was there. No per-item edit, no id, no delta protocol.
:::

:::concept{term="plan mode"}
A complete collaboration protocol built from the same primitives: a `/plan` entry command, a `plan:policy` prompt section, an `exit_plan_mode` tool, and a reviewed human decision gating the transition. Soft guidance, not enforcement.
:::

:::concept{term="whole-value-replace fold"}
Last write wins over a single `SessionEventMap` member (`todo/write`, `plan/mode`). No merging and no edit history — the latest logged event *is* the whole state, so resume, fork, and compaction all recover it with no live mirror.
:::

:::concept{term="session projection"}
A fold unit registered on `ctx.sessionProjections` that turns log events into live UI state (the web client's `TodoPanel` task strip, the plan-mode `{ active, pending }` indicator). Both packages consume this `core` registry; neither renders anything itself.
:::
...
`allowParallelInProgress` is required with no default — the README calls this out explicitly as a deployment choice, not a fixed rule, because whether concurrent active tasks are legitimate depends on runtime concurrency (parallel subagents, background jobs) the tool itself cannot observe. The flag changes two things together: the model-facing description (`DESCRIPTION_PARALLEL` vs. `DESCRIPTION_SINGLE` in `index.ts`) and the accepted input.

> [!NOTE]
> The flag does *not* change the durable-log invariant — `packages/todo/tool-todo/src/invariant.ts` deliberately stays silent on the active count, because a log written while parallel work was allowed must still replay after a later deployment tightens the policy.

The data path from call to rendered strip is a clean ordered flow:

:::timeline
- model calls `todo_write` — sends the complete list as arguments
- `toTodoList()` — validates; rejects empty/duplicate content, enforces the active-count policy
- reject non-agent caller — no owning session has nowhere to write the list
- append `todo/write` — the whole snapshot goes on the session log; excluded from `SurfaceEventType`
- projection folds — `todos` unit takes the new list (last write wins)
- UI renders — `TodoPanel` shows the standing plan; the model only sees a small acknowledgement
- `turn/start` — the strip clears once a genuinely new turn begins
:::
...
That's everything the model gets back in its own transcript. The full list — the thing a UI actually displays — lives entirely in the `todo/write` event, which the model never re-reads.

> [!NOTE]
> This split matters for the token story: the *call arguments* (the whole list the model sent) remain in history until compaction and grow with every write, but the *result* stays small and fixed-shape regardless of list length.
...
The fold is exactly two rules: take the whole list on `todo/write` (last write wins, per the whole-value-replace pattern), and reset to `null` on the next `turn/start`. Everything else — including `turn/end` — passes the state through unchanged. UIs — the web client's `TodoPanel`, mounted through the conversation input dock — subscribe to this projection and render the "standing plan" themselves; the tool package never renders anything.

:::fold[Why the strip clears on turn/start, not turn/end]
This is a deliberate lifetime decision, recorded in the [Todo plan strip clears on the next turn Agent Note](../../../.agents/notes/implemented/feature/2026-07-28-todo-plan-clears-on-next-turn.md): clearing on `turn/end` would hide the checklist while the user is still reading the just-finished answer, so the finished list stays visible through the end of the turn and disappears only once a new turn actually begins.
:::
...
Where `todo_write` is a small, single-purpose snapshot, `@deepseek-ai/dsh-plan-mode` is a complete collaboration protocol built from the same primitives. What plan mode owns is a single durable fact, one prompt section, and a reviewed transition.

> [!LIMITATION]
> Plan mode is **soft guidance, not enforcement**. Sandbox mode and approval policy — the mechanisms from [Approval and Questions](../s13-approval-and-questions/README.md) — restrict what a tool call may actually do, entirely independently; plan mode neither reads nor writes either of them.
...
The deployment supplies `section` — free text such as *"You are in plan mode. Explore and design before presenting the complete plan through exit_plan_mode."* — and that exact text renders at [system prompt](../s06-tool-pipeline-and-prompt/README.md) order 50 while plan mode is active, contributing nothing when inactive.

> [!NOTE]
> This is the entire mechanism by which plan mode changes model behavior: an instruction in the prompt, not a tool filter or sandbox cap. A model that ignores the guidance can still call any tool it always could — `exit_plan_mode` stays registered in both states precisely so the request's tool catalog never changes shape across the transition, keeping catalog structure stable across a mode flip.
...
:::fold[Narration: telling the model exactly when, never redundantly]
