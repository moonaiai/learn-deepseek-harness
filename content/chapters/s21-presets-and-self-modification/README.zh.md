...
本章讲两套机制，让同一个 Cordis 进程能对不同会话、以及同一会话内不同时刻，表现得各不相同。按本课程的定义，两者都不是能力 seam——一个 Service Definition 加上一个或多个 Service Provider 再加一个 Consumer，彼此可独立替换而不影响其余部分。生成的[`docs/capability-seams.md`](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/docs/capability-seams.md)会给每一个 `ctx.<key>` 行标注 `Role`,它直接给出了答案:`ctx.agentPresets`、`ctx.dynamicCordisRunner`、`ctx.cordisInspect` 全都被标记为 `core`,从未被标为 `seam`。

这个归类不是需要绕开的疏漏。这两套机制背后都没有一族可互换的后端实现:

1. **Agent preset**(`packages/preset`)背后没有一套可替换的 Definition/Provider 二人组来实现"preset 如何组装"——只有一个 `AgentPresets` 服务、一套挂载算法,没有第二种实现可供部署替换。真正变化的是 preset 所指向的那份 `cordis.yml` 的**内容**,而不是挂载机制本身。
2. **自我引用的 Cordis 工具集**(`packages/extensions`)背后同样没有一族可插拔的沙箱实现——只有一种 `node:vm` realm 形态、一套 guard、一个注册表。真正变化的是某个会话往里挂载的模型写代码。

两者都是直接建在 Cordis 与 `dsh-scope` 本就提供的原语之上——注册 scope 分层、`Include`/`group` 这套 loader 词汇、fiber 生命周期——而不是自成一套 seam。preset 是一个组装点,*使用*这些原语让某个会话拥有不同的工具集;工具集是一个内省与挂载点,用完全相同的原语让某一轮对话为同一会话的工具集做实时扩展。理解两者,主要就是看清楚这些原语原本就在那里,然后追问:让整份 `cordis.yml` 可以被指向一个 agent scope(第一部分),或者让单独一轮对话可以触达它(第二部分),分别需要补上的最小新能力是什么。
...
`code` 与 `cordis` 各自是 `standard` 的一份**完整拷贝**外加一处增量,而不是补丁格式。`dsh-agent-presets` 包的「已知限制」直接点出了这项代价:"a copy is a snapshot that drifts"(拷贝是会漂移的快照)——这一层没有补丁语义(那属于 `dsh-bundle` 的 `cordis.patch.yml` 机制),因此升级部署不会传播进已有的拷贝;随部署交付的这套 preset 自己也接受这项代价,换来的是每份 preset 都能读一个文件就看懂全貌。

### 隔离是默认值,而且是被实测过的

preset 子树的挂载默认按会话进行:根据[按会话组装 preset 的 Agent Note](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/.agents/notes/implemented/architecture/2026-08-03-per-session-agent-presets.md),一份十二行组装每会话大约耗费 3ms、约 600KB。这个数字之所以重要,是因为它支撑了一项设计决策——隔离足够便宜,便宜到可以把它定为默认值而非要 preset 显式选择,于是一份由用户或 agent 写出的 preset 天然获得尽可能小的影响面。

真正拥有某个昂贵或进程级资源的 preset,可以用 Cordis 自身的 `isolate` 词汇主动选择共享。具名 label 是进程级全局的 realm——两棵子树写同一个 label 就解析到同一个实例;而 `isolate: <name>: true` 则让每个挂载它的会话各自拿到该名字的私有实例。`code` preset 的文件头注释直接写明了服务行的规则:
...
`dsh-agent-presets` 在挂载时直接拒绝一行没有 realm 保护的裸服务行,而不是让相撞悄无声息地在后面才暴露出来。
...
后来的一次改进,在不改变 preset 文件写法的前提下,改变了 preset 子树**存在的份数**。最初的按会话设计为每个会话挂载一个全新的 Entry。[常驻挂载 Agent Note](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/.agents/notes/implemented/architecture/2026-08-08-per-preset-standing-mounts.md)解释了这为什么会拖垮三个原本假设注册表表面是静态的宿主读取方:冷读的 `session.history` 找不到 presenter(每张卡片都退化成通用渲染器);投影(projections)块丢掉了 preset 注册的键(客户端把缺失的键当作能力缺失,直接清空对应行);Typert 网关在宿主根上解析 `goals`,得到 `service-unavailable`。
...
工具只是「agent 是什么」的一半。[`dsh-persona`](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/packages/preset/persona/README.md)是让 preset 能改变另一半——agent 身份——的那一行,因为[`dsh-system-prompt`](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/packages/core/system-prompt/README.md)本身把部署级人设当作自己的配置字段来拥有,并且在全进程只注册一次这个段落。没有一行属于自己 scope 的行,preset 就只能换掉 agent 的工具,却动不了它声明的身份。
...
模型或人都无法通过名单服务直接提交原始的 preset 文本。`ctx.agentPresets.copy(from, id, name?)` 是**唯一**的创作写操作:它把一份已有 preset 的整个目录——组装、元数据、skill 目录、资源文件——原样拷贝进第一个 `user` 信任级别的根目录,把权限重新收紧为仅所有者可读写,解引用符号链接,并重写 `preset.yml`——保留源的 `description`,但丢弃它的 `name` 与清单里的 `order`。没有任何调用方能直接提供组装文本,因此一次拷贝授予不出名单本来就没有的任何东西,而且拷贝出来的东西与源一样能加载。
...
这套 harness 里的一切都是 Cordis 插件——但在这套工具集出现之前,运行在那套插件运行时**内部**的 agent 完全无法看见或触碰它:它无法枚举周围的服务与事件,无法在会话中途为自己扩展一个新工具,也无法即兴组合出自己发明的能力。[自我引用工具集 Agent Note](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/.agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md)把这个设计问题精确地表述成"让模型运行代码"必然捆绑而来的三个正确性难题:
...
| 包 | 角色 | ctx 键 |
|---|---|---|
| [`tool-cordis`](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/packages/extensions/tool-cordis/README.md) | 面向模型的工具,注册到 `ctx.tools` | `ctx.tools` |
| [`cordis-host-runner`](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/packages/extensions/cordis-host-runner/README.md) | 定义注册表、host 半的 `node:vm` 沙箱,以及 request-run 往返 | `ctx.dynamicCordisRunner`、`ctx.cordisInspect` |
| `cordis-client-runner` | 双半包的浏览器半 | client 面 |
| `ui-cordis` | 操作全部定义的全局面板 | client 面 |

`tool-cordis` 与 `cordis-host-runner` 的拆分遵循 harness 里随处可见的那套可复用模式:runner 拥有沙箱、vm 生命周期与跨会话记账;工具包只拥有面向模型的动词与 schema。两者缺一不可——一份只装了 `tool-cordis`、没装 runner 的组装,永远无法激活这些工具。

### 模型今天实际看到的七个动词

这套工具集的设计文档早于目前实际交付的代码。上面引用的 `2026-07-08` Agent Note 仍然记录着一套更早的三工具形态(`cordis_inspect` / `cordis_mount` / `cordis_unmount`),而这套形态在实际的 `tool-cordis` 源码里已经被取代,Agent Note 却没有跟着改写——这份笔记的设计理由(沙箱语义、信任立场、生成式目录的论证)依然成立,但它的工具表已经过时。模型今天实际能调用的代码注册了七个工具,这一点直接确认自 `tool-cordis/src/index.ts`:

| 工具 | 作用 |
|---|---|
| `cordis_inspect_list` | 列出 Host 目前已知的每一个 Cordis Inspect Provider,包括 Host 本地的以及从某个 Client 页面同步来的最新清单——每一条都带平台、用途、方法、输入输出 schema |
| `cordis_inspect_query` | 运行某个 Inspect Provider 明确声明过的一次只读查询,按 `cordis_inspect_list` 给出的确切 `platform`/`provider`/`method` 执行 |
| `cordis_inspect_self` | 对当前会话拥有的动态 Cordis 对象做只读报告:仅给 Plugin 摘要;给定 `pluginId` 则附带版本指针与 Package 摘要;给定 `pluginId` + `packageId` 则给出确切源码与运行时诊断 |
| `cordis_define` | 在对两个半区做语法检查后,记录一个不可变的 Package(`name`、`purpose`、host 半 `code` 与/或浏览器半 `client`)。此时什么都不会运行——用户看到的是一张带启动控件的卡片 |
| `cordis_run` | 激活某一个确切的 Package:`mode: "run"` 用于首次激活、重启或回滚;`mode: "update"` 用于从当前 Package 切换到另一个 |
| `cordis_stop` | 拆除当前的 Run,同时保留 Plugin、每一个 Package 与版本指针,以便之后再次运行或更新 |
| `cordis_undefine` | 视需要先停止该 Plugin,再永久删除它以及每一个 Package、授权与版本指针 |

`cordis_inspect_list`/`_query` 取代了早期一套手工维护的检查设计;`cordis_define`/`cordis_run` 把过去一次挂载调用拆成了"先记录、再激活",这样一份定义就能先作为对话卡片被审阅,再决定是否执行。每个动词都是**按会话限定**的:一个包只在定义它的那个会话里可见、可控——尽管它实际运行在共享的 DSH 进程内,一旦运行起来也可能影响到其他会话。动态包只存在于进程内存里——它们不会产生任何插件文件,不安装任何东西,不改动任何 `cordis.yml` 或个人配置,不会在 `dsh` 重启后存活,也不会被自动提升为正式能力。想保留一次实验,就得让 agent 通过常规开发流程,把它实现成一个正规的插件或 bundle——这套工具集明确是为探测而生,不是为交付而生。

### 为什么 API 与事件目录是生成出来的,而不是手写的

`cordis_inspect_query` 针对 `Service`/`Event` provider 的查询,读取的并不是一张手工维护的参考表——早期版本恰恰试过这么做,Agent Note 记录了它被替换的原因:"手写表一旦签名有变就会漂移,而没有任何东西会拦下这种漂移。"取而代之的是 `tool-cordis/src/api-catalog.ts`,由生成 `docs/subsystems/*.md` 的同一套 AST 遍历所生成——本章引用的 extensions 子系统自身 Cordis API 文档,正是同一份产物。`pnpm run gen-cordis-api` 负责重新生成,`pnpm run verify-cordis-api` 在 `doc-sync` 里守住它的新鲜度,因此工作区任何地方对公开签名或 JSDoc 的改动,都不可能在模型所读的目录跟着更新之前先合入。

运行时,这次查询会把这份静态目录与**存活**的服务存储做交集:什么在运行,来自存储;每个服务**能做什么**,来自目录;一个目录没覆盖的存活服务(例如某个已挂载的动态包自己提供的服务)依然会被报告为可达,只是没有签名,而不会被悄悄省略。报告代码里还有两处判断,是刻意做在渲染逻辑里、而非直接来自原始反射数据的:只展示**可调用的方法**(不展示状态,也不展示插件之间那些包外观本就够不到的、以 symbol 为键的内部接缝);以及只点名一个已挂载的 host 半**真正能做到**的 `ctx.<key>` 读取——一套 `injectable` / `not-a-service` / `other-face` 的分类被固定并纳入门禁,防止一个新声明的键悄悄邀请模型去 `inject` 一个永远不会到来的东西。

### 沙箱语义:为正确性而设的容器,不是安全边界

动态包的 host 半以异步函数体的形式,运行在一个全新的 `node:vm` realm 里。沙箱全局对象刻意精简:
...
一个带标签、透传写入的 `console`;`harness.defineTool` / `harness.registerTool` 这对注册接口;一个全新 vm 上下文原本缺失的编码原语;以及对被扣下的 Node API(`require`、定时器家族、`fetch`)设置的可调用陷阱——调用即抛出一条指名对应 Cordis 替代方案的消息。只有函数形态的全局对象会被设陷阱——`process` 与 `Buffer` 保持 `undefined`,因此一次 `typeof` 特性探测只会得到不存在,而不会触发一个抛错的访问器。
...
以上这些都不是安全边界,Agent Note 明确指出,这些陷阱与外观存在是为了**正确性**——把模型写的代码引导到 Cordis 服务上、远离它本会猜错的、容易泄漏的 Node 内建对象——而不是为了容器化:
...
两份包 README 用几乎一致的措辞说了同一件事:`tool-cordis` 的 README 说要"把这套工具集当作 bash 访问权限来对待",`cordis-host-runner` 的 README 说要"把一个动态包当作 bash 访问权限来对待"。沙箱全局对象上的 host-realm 辅助函数依然可达,因此包代码原则上仍能绕过陷阱触及 Node。一个被挂载的插件可以用一个已注入服务、以 host 执行器本身的权限,触达真实的文件系统与网络服务。这套工具集的加载,与加一行 `bash` 工具同样审慎——是一项需要主动选用的开发能力,而非硬化过的默认项——这正是为什么只有 `cordis` preset 携带它,`standard` 从不携带。

### 真正划定影响范围边界的,是什么

既然沙箱不是安全边界,真正承担容纳工作的,是三个更窄、也更真实的机制:

- **普通的 Cordis fiber 生命周期,而非专门造的清理路径。** 每个被挂载的 host 半都作为一个子 fiber,运行在 runner 之下一个内部 `cordis-dynamic` group 里,因此常规的 fiber dispose——拆除任何插件子树所用的那套机制——同时处理手动的 `cordis_stop`/`cordis_undefine`,以及工具集卸载或进程重启。`startHostHalf` 在返回之前会等待这个子 fiber 完全落定;启动失败会 dispose 掉这个 fiber 并重新抛出错误,发生在 `cordis_run` 报告成功之前,而不是留下一个半注册的插件活着。
...
(这条注释仍然写的是 `cordis_mount`——这份组装自己的文件头注释,和那份 Agent Note 一样,都没有更新为现在的 `cordis_define`/`cordis_run` 动词名;但它所声明的信任框架,对当前这套工具名同样成立、一字不差。)
...
两套机制都不构成一种新的权限类型。preset 拥有的权限恰好等于它所命名的那些插件——`AgentPreset` 上的 `trust` 字段存在的意义是让界面**展示**这一事实,而不是在此之上强制任何额外约束;`cordis_run` 挂载的动态包所拥有的权限,也恰好等于挂载它的会话所用的那份 preset 本就已经向它暴露的那些服务。两者真正添加的,是比"用另一份 `cordis.yml` 重启进程"更细的组合粒度:preset 这一层是按会话的粒度,自我修改这一层是按轮次的粒度。而按照[`docs/architecture.md` 的扩展点表格](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/docs/architecture.md#where-new-behavior-goes),"让某个会话拥有不同的能力集合"被记录为恰好一行——"组装一个 agent preset;其中的服务行需要 `isolate` realm"——与 harness 里其余每一个扩展点并列,无论它是不是 seam。
