...
summary: "Two collaboration mechanisms built from ordinary session events — the todo_write tool's whole-list task snapshot, and plan mode's logged propose/review/exit cycle around exit_plan_mode"
module: collab
order: 11
sources:
...
## Two seams for visible collaboration, not side effects

Every capability chapter so far has described a tool that *does* something in the world: it runs a command, reads a file, waits for approval. `todo_write` and plan mode are different in kind. Neither tool has an external side effect the model couldn't produce with words in a text response. What they buy instead is **visible, durable, structured state that a human (or a UI) can watch alongside the conversation** — a task list that persists as a strip in the interface, and a mode flag that changes what the model is instructed to do before it acts. Both mechanisms are built entirely from primitives earlier chapters already established: a `SessionEventMap` member, whole-value-replace folding, a `ctx.systemPrompt` section, and (for plan mode) the `ctx.userQuestions` review seam from [Permissions and Approval](../s09-permissions-and-approval/README.md). No new loop machinery is required for either.

They also make an instructive contrast with each other. `todo_write` is a single small tool package with one job: log a snapshot. Plan mode is a whole collaboration protocol — a prompt section, an entry command, an exit tool, and a reviewed human decision gating the transition. Reading them together shows the two ends of what "collaboration state" can mean in this harness.
...
The deployment supplies `section` — free text such as *"You are in plan mode. Explore and design before presenting the complete plan through exit_plan_mode."* — and that exact text renders at [system prompt](../s07-system-prompt/README.md) order 50 while plan mode is active, contributing nothing when inactive. This is the entire mechanism by which plan mode changes model behavior: an instruction in the prompt, not a tool filter or sandbox cap. A model that ignores the guidance can still call any tool it always could — `exit_plan_mode` stays registered in both states precisely so the request's tool catalog never changes shape across the transition, keeping catalog structure stable across a mode flip.
...
2. Ask through `ctx.userQuestions` — the exact seam described in [Permissions and Approval](../s09-permissions-and-approval/README.md) — with the plan text as `detail` and a `plan-review` presentation `intent` naming `Approve` as the label that consents:
