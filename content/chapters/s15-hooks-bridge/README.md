...
summary: "Why hooks-claude-code/hooks-codex are not a capability seam of their own but Consumers of ctx.shell and ctx.sessionPersistence, sharing dsh-hook-protocol's dialect-neutral matcher/codec/merge/runner primitives to translate external hooks.json configs onto the harness's typed interception points"
...
sources:
  ...
  - path: docs/capability-seams.md
    lineStart: 448
    lineEnd: 448
    label: "ctx.shell row: hooks-claude-code/hooks-codex as direct consumers"
  - path: docs/capability-seams.md
    lineStart: 422
    lineEnd: 422
    label: "ctx.sessionPersistence row: hooks-claude-code/hooks-codex as direct consumers"
  - path: packages/hooks/hooks-claude-code/src/index.ts
    lineStart: 39
    lineEnd: 42
    label: "name/inject = ['shell'] — namespace plugin, no default export"
