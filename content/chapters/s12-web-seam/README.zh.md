...
`ctx.web` 是 agent 访问 web 的入口;与 shell 或 LLM 不同,它有意在**一个 seam 上承载两种操作**:`search` 与 `fetch`。一个具体的注册表(`WebRuntime`）为每种操作容纳任意数量的提供方,因此三个互相竞争的搜索厂商与一个抓取后端可以并列挂在同一进程里。选择并非在启动时完成——`resolveProvider` 在每次调用时重新执行,并拒绝在多个可用提供方之间猜测。`dsh-tool-web` 是唯一稳定的面向模型 Consumer，从不导入任何提供方包。下文讲清为什么两种操作共享一个 seam、为什么选择发生在执行时、为什么三个搜索厂商正是设计目的而非冗余。

## 速览

:::concept{term="ctx.web / WebRuntime"}
Cordis `Service`——本章的 Service Definition。一个持有两个提供方 `Map`（一个给 search、一个给 fetch）的具体类，外加每次调用执行的选择策略。`dsh-tool-web` 调的就是它。
:::

:::concept{term="WebSearchProvider / WebFetchProvider"}
提供方契约：一个带 `id`、`available()` 探测和 `search()`/`fetch()` 的普通对象。没有任何提供方去继承 `WebRuntime`；每个都被注册进去，并随贡献它的 fiber 一并释放。
:::

:::concept{term="resolveProvider"}
选择策略，在每次调用时重新执行，而非启动时选定一次。它把已配置的 id（或未配置时的可用提供方数量）与每个提供方本地的 `available()` 检查配对——面对多个可用者时宁可抛错也不猜测。
:::

:::concept{term="available()"}
一个同步的、仅限本地的探测——凭据是否存在、端点配置是否可解析。它回答"这个具体实现现在是否可用"，而非"网络现在是否可达"，并且禁止在其中发起网络调用。
:::

:::concept{term="WebError"}
该 seam 唯一的错误/中止词汇：结构化错误码（`WEB_DUPLICATE_PROVIDER`、`WEB_PROVIDER_AMBIGUOUS`、`WEB_PROVIDER_UNAVAILABLE`……)，模型把它当作普通的工具错误来读取。
:::
...
把这段函数读作一个决策过程，它在每次调用时只走一条有序路径：

:::timeline
- 已配置 id —— 在提供方 map 中查它
- id 未注册 —— 抛 `WEB_PROVIDER_CONFIGURED_MISSING`
- 已注册但 `available()` 为 false —— 抛 `WEB_PROVIDER_CONFIGURED_UNAVAILABLE`
- 已注册且可用 —— 选中它；结束
- 未配置 id —— 只保留 `available()` 通过的提供方
- 零个可用 —— 抛 `WEB_PROVIDER_UNAVAILABLE`
- 多个可用 —— 抛 `WEB_PROVIDER_AMBIGUOUS`，列出每一个 id
- 恰好一个可用 —— 选中它；结束
:::
...
因此,一份 `cordis.yml` 组合确实可以在同一进程中并列挂载 `dsh-web-search-exa`、`dsh-web-search-perplexity` 和 `dsh-web-search-deepseek`——Loader 或 seam 都不会拒绝这种组合——但除非 `searchProvider`(或 `$DSH_WEB_SEARCH_PROVIDER`)恰好指定唯一一个,否则 `ctx.web.search()` 会拒绝在它们之间猜测。这与"用一个提供方替换另一个"恰恰相反:这是"挂载多个,再用一个配置旋钮指向获胜者",而 seam 会主动拒绝歧义情形,而不是按注册顺序挑选。

> [!WHY]
> [Agent Note 明确点名了这个替代方案并否决了它](../../../.agents/notes/implemented/architecture/2026-06-24-web-capability-seam.md#alternatives-considered):"注册顺序不是产品策略。它可能因配置顺序、插件加载、HMR 或重构而改变。"
...
抓取一个已知 URL 和搜索一个未知 URL 是两个不同的问题,有着不同的安全姿态。搜索返回的是*引用*——提供方已经爬取过的页面、由提供方控制的摘录。抓取直接获取*调用方任意提供的 URL*,这使得 `web_fetch` 成为一个真实的网络边界。

> [!LIMITATION]
> [`dsh-web-fetch-http` 的 README](../../../packages/web/web-fetch-http/README.md) 明确指出,在私有网络屏蔽落地之前,"该提供方是一个 SSRF 原语";目前只交付了同源重定向跟随、字节／字符／URL 长度上限、URL 中凭据拒绝,以及真实的 `User-Agent`——没有先解析 DNS 再验证、没有逐跳重新验证,也不屏蔽 loopback／link-local／私有网段。

这些传输卫生措施没有一项与搜索有关;搜索提供方从不接触模型调用方提供的 URL。
...
有两处省略,对正确理解该 seam 的其余部分很重要——两处都是刻意的,并且都记录在 Agent Note 里。

:::fold[没有状态查询,以及一个刻意做薄的 WebSearchRequest]
第一,没有能力状态查询,也没有提供方变更事件:调用方只能通过执行并按抛出的 `WebError` code 路由来"观测"可用性——[seam 的已知限制](../../../packages/web/web/README.md)直接点明了这一点,并引用了 [已废弃的观测接口 Agent Note](../../../.agents/notes/archived/simplification/2026-07-04-drop-unconsumed-web-observation-surface.md),记录了为什么从未有消费方需要这样的接口。

第二,`WebSearchRequest` 只携带 `query` 和 `maxResults`——没有任何新近程度窗口、域名过滤条件或搜索深度控制能到达 seam,尽管 Perplexity 的 `searchRecency` 与 Exa 的 `searchType`／`highlightsPerResult` 确实作为*提供方私有*配置字段存在。只有当每一个当前的搜索提供方都能诚实支持某个提供方无关字段时,该字段才会被加入;在那之前,提供方专属的旋钮会留在共享词汇之外,而不是作为只有部分提供方会填充的可选字段泄漏进来。
:::
