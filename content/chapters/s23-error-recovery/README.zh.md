...
当一个正在运行的 agent 出问题时——用户停掉一个轮次、某个工具调用在执行途中被打断、一次模型请求返回了传输层错误——这套 harness 会动用两套机制之一。**取消**是刻意的：循环被*请求*停止工作，并一直等到那份工作真正停稳，绝不硬杀。**重试**是自动的：某个插件透明地重发一次失败的模型请求，模型和用户都看不到这次失败。两者都依托于这个代码库反复推演出的同一条规则：**永远不要放弃一份你无法证明已经停止的工作。** 本章接下来会把两套机制对照代码讲清，最后以三篇真实的事故复盘收尾——那些真正上线过的失败，以及它们各自留下的防御性规则。

## 速览

三个词撑起整章。前两个是驱动器围绕其构建的两种失败模式；第三个是它们共同遵守的承诺。

:::concept{term="取消（Cancellation）"}
刻意的。某个主体——用户、父 agent、hook、dispose——决定某个轮次或工具调用应当停止，循环与这个决定*协作*，而不是强行杀死正在进行的工作。
:::

:::concept{term="重试（Retry）"}
自动的。一次模型请求以某种「重试可能会成功」的方式失败了，某个插件决定是否透明地重试它——模型和用户都不会看到。
:::

:::concept{term="请求停止，而非强杀"}
共同的承诺：永远不要放弃一份你无法证明已经停止的工作。被取消的轮次不会在它打断的工作真正停稳之前返回「已取消」；被重试的请求也不会悄悄重复一次工具调用。
:::

这正是 [`docs/defensive-patterns.md`](../../../../deepseek-harness/docs/defensive-patterns.md) 里那条通用规则——「dispose 必须达到完全停稳，而不仅仅是请求停止」——被具体应用在轮次/步骤驱动器上的样子。本章后半部分把这条规则反过来用：既然失败总会发生，这个代码库究竟如何从真正上线过的失败中吸取教训？
...
:::decision
我们对取消场景拒绝 `Promise.race` 式的提前放弃。在 JavaScript 里赛跑本是一个现成的*选项*，而这个代码库明确拒绝了它：提前放弃一个正在进行的 Promise，会让驱动器在被放弃的工作的副作用仍在落地——某个工具还在写文件、某个子进程还在运行——的时候就上报 `idle`。不配合的同进程工作会拖慢一次取消真正生效可见的速度（这个取舍是真实的），但上报出来的「已停稳」状态始终是真实的。真正卡死的工作需要硬性终止，那是 worker 或进程隔离边界的职责——不在这个控制机制的范围内。
:::

:::fold[signal 触达的范围——以及它从不经过的路径]
...
:::decision
在类型层面把 signal 设为必需。这体现了贯穿整个代码库的「显式优于隐式」惯例：可选的 signal 会允许某个调用方悄悄跳过取消机制，而注册表也无法合成一个代表它并不拥有的生命周期的兜底 signal。把它设为必需，把「这个工具从未被正确取消过」这一类 bug 从运行时意外变成了编译期错误。
:::

:::fold[signal 可变性跟随流水线阶段]
...
| Code | 何时触发 | 模型可见文本 |
|---|---|---|
| `TOOL_ABORTED_BEFORE_DISPATCH` | 取消发生在工具体被调用之前，工具体从未被触发 | `Error: tool call aborted before dispatch` |
| `TOOL_ABORTED` | 取消发生在工具体已经被调用*之后* | `Error: tool call aborted` |
...
这个循环是一个固定的、有序的周期。把它当成一步步的流程来走：

:::timeline
- `buildRequest()` 从持久历史组装一个冻结的请求
- 流式读取分片，直到 `assembler.finish` 落定
- finish 是 `error` 或 `aborted` → 触发 `agent/request-error` waterfall（此时轮次 signal 仍然存活）
- 某个监听器返回 `{ kind: 'retry' }` 且不调用 `next()` → 短路，直接回到 `continue`
- 循环从同一份持久历史重建请求，作为同一轮次内的一个新步骤重发
- 没有监听器重试 → 默认终端处理器返回 `undefined` → 抛出 `LlmError`，同时关闭步骤与轮次
:::
...
> [!PITFALL]
> 178 个绿色单元测试和 100% 行覆盖率——然而这个 bug 只存在于真实 Loader 路径上，而没有任何手动挂载的测试走到过那条路径。
...
> [!PITFALL]
> 快照测试套件之所以通过，是把这次失败记录成了新的预期输出——那是生产 fixture，不是正确性审查。
...
> [!PITFALL]
> 假沙箱 provider 从未产生过「无害通知 + 子进程自行决定的非零退出」这种组合，而真实内核测试会自我跳过——触发这个 bug 所需的确切 ABI 条件在大多数 CI 宿主上根本没有被测试到。
