...
summary: A guided run of pnpm dsh --profile headless "task", tracing profile composition,
  one real turn, the mounted tool set, subagent/workflow delegation, and persisted
  output back to the concepts from every earlier chapter.
...
What differs between deployments is which capability seams (Chapter s07) are actually composed, and `examples/headless-agent/cordis.yml`'s comments spell out the dependency chains plainly:
...
| `advanced.cordis.yml` | Repoints the agent at `deepseek-v4-pro`, sets `tools.mode: both`, and inserts `dsh-code-runtime-worker-thread` + `dsh-cordis-host-runner` + `dsh-tool-cordis` — Code Mode plus the self-modification tool from Chapter s21 |
...
| `pty.cordis.snapshot.yml` | Inserts `dsh-terminal` + `dsh-tool-terminal` under `danger-full-access` — the persistent terminal seam from Chapter s09, off by default |
...
## Where persistence actually writes (back to s19)
