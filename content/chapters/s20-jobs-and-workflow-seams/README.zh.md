...
title: "任务与工作流接缝"
summary: "ctx.jobs 和 ctx.workflowEngine 是真正的能力接缝,拥有 Definition/Provider/Consumer 三种角色,分别对应即发即弃的后台工作和 worker-thread 编排(及其固定策略 Ralph);Schedule 与它们外形相似,却明确不是接缝——没有 ctx 键,持久状态直接写在会话日志里。"
...
sources:
  - path: docs/capability-seams.md
    lineStart: 459
    lineEnd: 459
    label: "ctx.jobs seam 行(生成)"
  - path: docs/capability-seams.md
    lineStart: 465
    lineEnd: 465
    label: "ctx.workflowEngine seam 行(生成)"
  ...
  - path: packages/schedule/README.zh.md
    lineStart: 1
    lineEnd: 13
    label: "schedule/ 家族 README(中文)——明确没有 ctx 键、没有公开 service"
