...
The next several chapters each take one specific seam and go deep — filesystem and LSP, subprocess and terminal, sandbox, the LLM seam, the web seam — and later chapters cover subagents, skills, compaction, and session persistence as seams with especially wide provider fan-out. Not every mechanism in this codebase is a seam, and later chapters will say so explicitly where that's the case.
