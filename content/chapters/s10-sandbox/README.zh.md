...
[`dsh-sandbox`](../../../packages/sandbox/sandbox/README.md) 拥有 `ctx.sandbox` 与共享的限制词汇:`SandboxMode`(`read-only` / `workspace-write` / `danger-full-access`,仅限文件操作)、`SandboxEnforcement`(`full` / `partial`)、`SandboxExecutionPolicy` / `SandboxPolicy`(完整的逐调用模式加工作区根目录),以及故障时拒绝放行的 `SANDBOX_UNAVAILABLE` 错误。它只依赖 Cordis 与 harness 错误基类——绝不依赖后端。
...
- **`PLATFORM_CHAINS` 中不存在的平台,或链表中每个候选项都探测为 `unusable`,都会以 `SandboxUnavailableError` 与 `SANDBOX_UNAVAILABLE` 错误码拒绝放行**——绝不会静默地无限制通过。
