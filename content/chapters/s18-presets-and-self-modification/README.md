...
summary: "How a preset cordis.yml composes one session's tools and persona without touching any other session, and how the self-referential Cordis toolset lets an agent inspect and mount plugins in its own live runtime — through the same registration system, not arbitrary code execution."
module: ops
order: 18
sources:
  - path: packages/preset/README.md
    label: "preset/ package group overview"
  - path: packages/preset/agent-presets/README.md
    label: "dsh-agent-presets: the AgentPresets service"
  - path: packages/preset/persona/README.md
    label: "dsh-persona: the agent persona as a composable row"
  - path: .agents/notes/implemented/architecture/2026-08-03-per-session-agent-presets.md
    label: "Agent Note: a session's agent is composed from a preset cordis.yml"
  - path: .agents/notes/implemented/architecture/2026-08-08-per-preset-standing-mounts.md
    label: "Agent Note: per-preset standing mounts over a scope parent chain"
  - path: packages/extensions/README.md
    label: "extensions/ package group overview"
  - path: docs/subsystems/extensions.md
    label: "Generated extensions subsystem doc (Cordis API)"
  - path: packages/extensions/tool-cordis/README.md
    label: "dsh-tool-cordis: the five model-facing tools"
  - path: packages/extensions/cordis-host-runner/README.md
    label: "dsh-cordis-host-runner: definition registry and vm sandbox"
  - path: .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md
    label: "Agent Note: the self-referential cordis toolset"
  - path: apps/cli/config/agent-presets/minimal/agent.cordis.yml
    label: "The minimal preset: two tools, a complete persona"
  - path: apps/cli/config/agent-presets/code/agent.cordis.yml
    label: "The code preset: standard plus Code Mode presentation"
  - path: apps/cli/config/agent-presets/cordis/agent.cordis.yml
    lineStart: 1
    lineEnd: 30
    label: "The cordis preset: standard plus self-modification"
  - path: apps/cli/config/agent-presets/cordis/agent.cordis.yml
    lineStart: 241
    lineEnd: 262
    label: "The cordis preset: tool-cordis and the authoring skill"
  - path: apps/cli/config/agent-presets/cordis/skills/editing-cordis-compositions/SKILL.md
    label: "Skill: editing Cordis compositions"
  - path: docs/architecture.md
    lineStart: 106
    lineEnd: 112
    label: "\"Where new behavior goes\" table, preset row"
---

## Two questions, one mechanism

This chapter answers two questions that sound unrelated but are solved with the same trick.

1. **How does one `dsh` process run several agents with different tool sets and personas at once**, without spinning up a process per agent? That's `packages/preset`.
2. **How does the agent inspect and change the plugin tree it is itself running inside**, at runtime, without a restart? That's `packages/extensions`.

Both answers reduce to a fact stated plainly by the [per-session preset Agent Note](../../../.agents/notes/implemented/architecture/2026-08-03-per-session-agent-presets.md): `dsh-tools` and `dsh-system-prompt` already file every registration into the *calling context's scope layer*, and an agent is itself a registration scope. Nothing about the loader needs to change to give one agent its own tools — you only need a way to point a whole `cordis.yml` at one agent's scope context, and a way to let the model itself add or remove rows from a scope while it runs. A preset is the first. The self-referential toolset is the second, applied to the model's own turn instead of to session startup.
...
`code` and `cordis` are each a *full copy* of `standard` with one addition layered on top, not a diff format. The [preset Agent Note](../../../.agents/notes/implemented/architecture/2026-08-03-per-session-agent-presets.md)'s Known Limitations name this cost directly: "a copy is a snapshot that drifts" — there is no patch semantics at this layer (that belongs to the `dsh-bundle` `cordis.patch.yml` mechanism), so upgrading the deployment does not propagate into copies, and the shipped set accepts that cost itself so the whole assembly stays readable in one file per preset.

### Isolation is the default, and it is measured

Mounting a preset subtree is per-session by default: a twelve-row composition costs roughly 3ms and 600KB per session in the original per-session design. That number matters for a design decision — isolation is cheap enough to make it the default rather than something a preset has to opt into, so a preset authored by a user or by an agent gets the smallest possible blast radius automatically.

A preset that genuinely owns something expensive or process-scoped opts *into* sharing with Cordis's own `isolate` vocabulary. A named label is a process-global realm — two subtrees naming the same label resolve to the same instance — while `isolate: <name>: true` gives each mounting session its own private instance of that name. The `code` preset's comments spell out the rule for a service row: it "MUST sit inside a group carrying an `isolate` realm. Without one it publishes into the root realm, where it is process-global rather than per-session and the second session mounting this preset collides with the first" — and `dsh-agent-presets` rejects that at mount time rather than letting the collision surface as a silent, unobserved rejection later.
...
Tools are only half of "what an agent is." [`dsh-persona`](../../../packages/preset/persona/README.md) is the row that lets a preset change the other half — the agent's identity — because [`dsh-system-prompt`](../../../packages/core/system-prompt/README.md) owns the deployment persona unconditionally and registers it once, process-wide. Without a scoped row of its own, a preset could swap an agent's tools but never its stated identity.
...
Everything in this harness is a Cordis plugin — but until this toolset existed, the agent running *inside* that plugin runtime had no way to see or touch it. It could not enumerate the services and events around it, extend itself with a new tool mid-session, or compose capabilities it invented on the spot. The [self-referential toolset Agent Note](../../../.agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md) frames the design problem precisely as three correctness hazards that come bundled with "let the model run code":
...
|---|---|---|
| [`tool-cordis`](../../../packages/extensions/tool-cordis/README.md) | Model-facing tools: `cordis_inspect`, `cordis_define`, `cordis_run`, `cordis_stop`, `cordis_undefine` | registers on `ctx.tools` |
| [`cordis-host-runner`](../../../packages/extensions/cordis-host-runner/README.md) | The definition registry, the `node:vm` sandbox for host halves, and the request-run round trip | provides `ctx.dynamicCordisRunner` |
...
`tool-cordis` and `cordis-host-runner` split cleanly along the capability-seam pattern used everywhere else in the harness: the runner owns the sandbox, the vm lifecycle, and cross-session bookkeeping; the tool package owns only the model-facing verbs and schemas. Neither package is useful without the other — a composition with `tool-cordis` but no runner never activates its tools.

### The five verbs

The tool surface evolved from an earlier three-verb design (`cordis_inspect` / `cordis_mount` / `cordis_unmount`) into today's five, splitting "define" from "run" so a definition can be recorded, reviewed by a person as a conversation card, and started later — or never:

| Tool | What it does |
|---|---|
| `cordis_inspect` | Read-only report over the live process: services, every live plugin fiber, registered tools, this session's dynamic packages, the reflection-backed `api`/`events` references, and the compile-time `client` slot surface a browser half can contribute UI into |
| `cordis_define` | Records a package (`name`, `purpose`, a host-half `code` and/or a browser-half `client`) after syntax-checking both halves. Nothing runs yet — the user sees a card with a start control |
| `cordis_run` | Evaluates the host half in the sandbox and delivers the browser half to every open web page |
...
| `cordis_undefine` | Stops the package if needed and forgets the definition |
...
None of this is a security boundary, and both README files say so in the same words, deliberately: *"Treat this toolset like bash access."* The [Agent Note](../../../.agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md) is explicit that the traps and façade are there for **correctness** — steering model-written code onto Cordis services and away from leak-prone Node built-ins it would otherwise guess wrong about — not for containment:
...
Host-realm helpers on the sandbox global remain reachable, so package code can, in principle, reach Node despite the traps. A mounted plugin can call `ctx.shell` with the host executor's own privileges and reach the real filesystem and web services. This toolset is loaded exactly as deliberately as a `bash` tool row — an opt-in development capability, not a hardened default — and that is why only the `cordis` preset carries it, never `standard`.
...
- **Ordinary Cordis fiber lifecycle, not a bespoke cleanup path.** Every mounted plugin is a child of one internal group beneath the tool plugin, so ordinary fiber disposal — the same mechanism that tears down any plugin subtree — handles both a manual `cordis_stop`/`cordis_undefine` and toolset unload or process restart. `cordis_run` awaits full settlement; a startup failure disposes the fiber before returning an error rather than leaving a half-registered plugin live.
