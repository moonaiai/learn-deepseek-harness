...
- `ctx.shell`(`seam`,由 `dsh-shell` 拥有,由 `dsh-bash-local`/`dsh-bash-sandbox`/`dsh-pwsh-local` 实现)把 `hooks-claude-code` 和 `hooks-codex` 与 `tool-bash`/`tool-pwsh` 并列为直接 consumer——见 `docs/capability-seams.md:448`。
- `ctx.sessionPersistence`(`seam`,由 `dsh-session-persistence` 拥有,由 JSONL/SQLite 两个后端实现)把这两个桥接与 `agent-loop`/`tool-bash`/`session-query`/`message-feedback` 并列——见 `docs/capability-seams.md:422`。

两个桥接不仅在生成图谱里如此,源码里也是这么声明的:`export const inject = ['shell']` 是每个插件 `apply()` 唯一强制要求的注入(`hooks-claude-code/src/index.ts:39-42`,`hooks-codex/src/index.ts` 与之对称)。`sessionPersistence` 则是通过 `ctx.get('sessionPersistence')` 机会性地读取,而不是注入——即使没有加载任何持久化后端,钩子依然能运行,只是 `transcript_path`/`session_id` 这个定位字段会是空值,这正是 harness 里「不该成为某个本应独立的插件的硬依赖的可选服务」所遵循的通用模式。

所以 hooks 家族有一种**双重身份**:`dsh-hook-protocol` 是一个共享库(不向 Cordis 注册,不注入任何东西,自然也谈不上是谁的 Consumer),而两个桥接插件则是叠加在两个互不相关的 seam 之上的普通 Consumer——一个用于运行一条命令(`ctx.shell`),一个用于定位会话的持久化日志路径,以便写进钩子的 stdin payload(`ctx.sessionPersistence`)。两个桥接都不拥有自己的 Service Definition,harness 里也没有任何机制打算把「这个 hooks 桥接」换成另一种实现,就像沙箱执行器替换本地执行器那样。接下来要讲的,正是为什么这种形态是正确的,而不是一个缺口。
...
flowchart LR
  ...
  subgraph bridge["桥接插件（方言相关的 Consumer）"]
  ...
  codexbridge --> runner
  runner --> shellseam
  ccbridge -.->|ctx.get，可选| persistseam
  codexbridge -.->|ctx.get，可选| persistseam
  runner --> codec
  codec --> merge
  merge --> ccbridge
  merge --> codexbridge
...
## `ctx.sessionPersistence` 在这里扮演的角色

两个桥接的 agent 相关 stdin payload 都携带 `session_id` 与一个 `transcript_path` 形状的字段,因为真实的 CC/Codex 钩子脚本普遍会读取这个字段来定位磁盘上的会话记录。两个桥接都不注入 `sessionPersistence`,而是各自机会性地通过 `ctx.get('sessionPersistence')?.locate(agent.session.header)?.path` 解析,在没有加载持久化后端时回退为 `''`(CC)或 `null`(Codex,保留其 `string | null` 的线上格式)。`locate()` 不会创建或落盘该文件,所以返回的路径可能在第一次 turn-end 检查点之前就是缺失的,也可能不包含当前尚未持久化的这一回合内容。这正是开篇双重身份框架指向的第二个 seam:两个桥接消费 `ctx.sessionPersistence` 的方式,和 `session-query`/`message-feedback` 完全一样——只读、尽力而为,从不是这个服务的拥有者。
